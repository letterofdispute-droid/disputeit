...
// Map verticals to template category IDs
...
// Valid article types from the check constraint
...
serve(async (req) => {
  ...
  try {
    ...
    const { vertical, allVerticals } = await req.json() as { vertical?: string; allVerticals?: boolean };
    ...
    if (allVerticals) {
      ...
      if (verticalData) {
        verticalsToProcess = [...new Set(verticalData.map(d => d.vertical))];
      }
    } else if (vertical) {
    ...
    if (verticalsToProcess.length === 0) {
      return new Response(JSON.stringify({ success: true, message: 'No unused keywords found', planned: 0 }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    let totalPlanned = 0;
    const verticalResults: Record<string, { pillars: number; clusters: number }> = {};

    for (const vert of verticalsToProcess) {
      const categoryId = VERTICAL_TO_CATEGORY[vert.toLowerCase()] || vert.toLowerCase();
      
      // Fetch unused seed keywords
      ...
      // Fetch variation keywords grouped by column_group
      ...
      // Group variations by column_group
      ...
      // Build AI prompt with seed keywords and their variations
      const keywordContext = seedKeywords.map(seed => {
        const variations = variationsByGroup[seed.column_group || seed.keyword] || [];
        return `SEED: "${seed.keyword}" (${seed.column_group || 'standalone'})\n  Variations: ${variations.map(v => `"${v.keyword}"`).join(', ') || 'none'}`;
      }).join('\n\n');
  ...
  "pillars": [
    {
      ...
      if (!response.ok) {
        console.error(`[PLAN] AI error for ${vert}: ${response.status}`);
        continue;
      }
      ...
      if (!plan.pillars || !Array.isArray(plan.pillars)) {
        console.error(`[PLAN] Invalid plan structure for ${vert}`);
        continue;
      }
      ...
      for (const pillar of plan.pillars) {
        // Create content_plan for this pillar group
        ...
        const pillarType = VALID_ARTICLE_TYPES.includes(pillar.article_type) ? pillar.article_type : 'how-to';

        // Create pillar queue item
        const { data: pillarQueueItem, error: pillarError } = await supabase
          ...
          .insert({
            ...
            priority: 1, // Pillars generated last
            status: 'queued',
          })
          .select('id')
          .single();
        ...
        for (const cluster of (pillar.clusters || [])) {
          ...
          // Mark variation keyword as used
      ...
      verticalResults[vert] = { pillars: pillarCount, clusters: clusterCount };
      totalPlanned += pillarCount + clusterCount;
