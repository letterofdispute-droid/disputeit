...
export function RelatedTemplatesCTA({ templateSlugs, categorySlug }: RelatedTemplatesCTAProps) {
  ...
  const templates = templateSlugs
    ...
    .filter(Boolean)
    .slice(0, 3); // Show max 3 templates
  ...
  return (
    <div className="my-8 p-6 bg-gradient-to-br from-primary/5 to-accent/5 rounded-2xl border border-primary/10">
      ...
      <div className="space-y-3">
        {templates.map((template) => {
          ...
          return (
            <Card 
              key={template.slug}
              className="group hover:shadow-md transition-all duration-200 border-border/50 bg-background"
            >
              <CardContent className="p-4">
                <Link to={templateUrl} className="flex items-start gap-4">
